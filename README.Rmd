...
The interface is built with a consistent naming scheme and argument structure. The first argument will be the string or strings to work on, which makes it particularly convenient to work with pipes `|>`. Functions start with prefixes for easy identification and auto-completion. Get familiar with the design by reading `vignette("suitestrings-conventions")`:
